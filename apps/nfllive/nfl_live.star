...
def main(config):
    ...
    team_id = str(config.get("teamid", "2"))
...
def local_start(value, config):
    ...
    local = time.parse_time(value).in_location(timezone)
    fixture_now = config.get("_fixture_now", "")
    now = time.now() if fixture_now == "" else time.parse_time(fixture_now)
    if local.format("2006-01-02") == now.in_location(timezone).format("2006-01-02"):
...
def fixture_game(game_id, status, period, clock, away_score = 17, home_score = 21, scheduled = "2026-09-11T00:20:00Z", stale = False, tie = False):
    detail = ""
    if status == "live":
    ...
    return {
        ...
        "awayTeam": fixture_team(2, "BUF", "#00338D"),
        "homeTeam": fixture_team(17, "NE", "#002244"),
