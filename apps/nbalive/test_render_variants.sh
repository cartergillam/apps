...
out_dir="${TRONBYT_RENDER_OUTPUT:-$(mktemp -d "${TMPDIR:-/tmp}/tronbyt-nba-render-tests.XXXXXX")}"
mkdir -p "$out_dir"

for scenario in logo_missing large_scores tied next_worst off_day future scheduled pregame q1 q2 halftime q3 q4 ot double_ot final final_ot delayed postponed cancelled suspended unknown stale timezone_boundary empty worst_case; do
...
pixlet render apps/nbalive/nba_live.star '$sports_data={"league":"nba","games":null,"upcomingGames":null,"nextGame":null,"stale":null}' --output "$out_dir/null-collections.webp"
pixlet render apps/nbalive/nba_live.star '$sports_data={"league":"nba","games":[{"awayTeam":null,"homeTeam":{"abbreviation":null,"primaryColor":null,"logoData":null},"status":null,"awayScore":null,"homeScore":null,"awayRecord":null,"homeRecord":null,"scheduledAt":null,"scheduledLocal":null,"statusDetail":null,"stale":null}],"upcomingGames":null,"nextGame":null,"stale":null}' --output "$out_dir/null-game-fields.webp"
