"""NHL Overview: normalized server data, no provider or image HTTP."""
...
def main(config):
    ...
    if not pages:
        pages = [message("NHL OVERVIEW", "CHOOSE TEAM" if config.get("teamid", "10") == "" else "UNAVAILABLE")]
    if len(pages) == 1:
...
def get_schema():
    return schema.Schema(version = "1", fields = [
        schema.Dropdown(id = "teamid", name = "Favorite Team", desc = "Follow one team through its season, next game and last result.", icon = "star", default = "10", options = team_options()),
